...
def create_asset_with_content(connection, name, collection, input_file_path):
    """ Create an asset with specified name in the given asset collection.

    :param connection: Mediaflux server connection object
    :type connection: mfclient.MFConnection
    :param name: Name of the asset
    :type name: str
    :param collection: Destination asset collection
    :type collection: str
    ...
    w.add('pid', f"path={collection}", attributes={'create': True})  # the destination asset collection where the asset is created
    w.add('name', name)
    w.push('meta')
    w.push('mf-name')
    w.add('name', name)
...
if __name__ == '__main__':
    ...
    try:
        ...
        # create asset
        asset_id = create_asset_with_content(connection, 'Test1', '/projects/test', INPUT_FILE)
